...
class PassThrough:
    def __init__(self):
        self.longest_path = pynini.closure(
            pynutil.add_weight(NEMO_NOT_SPACE | NEMO_SPACE, 100)
        )
        self.word_fst = pynini.closure(NEMO_NOT_SPACE)
        self.sent_fst = pynini.closure(NEMO_SPACE | self.word_fst)
        self.phrase_fst = pynini.closure(
            pynini.closure(NEMO_NOT_SPACE) ** (2, ...) + NEMO_WHITE_SPACE
        )
        ...
        self.w3 = pynutil.add_weight(
            w_fst + pynini.closure(self.longest_path + w_fst), 1
        )
        ...
        self.phrase_fst = (
            pynini.closure(self.w1, 1)
            | pynini.closure(self.w2, 1)
            | pynini.closure(self.w3, 1)
        )
        self.phrase_fst |= pynini.closure(NEMO_WHITE_SPACE)
        self.phrase_fst |= self.longest_path
