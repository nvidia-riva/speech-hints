...
class PassThrough:
    def __init__(self):
        ...
        self.phrase_fst = pynini.closure(pynini.closure(NEMO_ALPHA)**(2,...) + NEMO_WHITE_SPACE)
        w_fst = pynini.closure(NEMO_ALPHA, 2)
        self.w1 = pynutil.add_weight(NEMO_ALPHA + NEMO_WHITE_SPACE + w_fst, 1)
        self.w2 = pynutil.add_weight(w_fst + NEMO_WHITE_SPACE + NEMO_ALPHA, 1)
        self.w3 = pynutil.add_weight(w_fst + pynini.closure(NEMO_WHITE_SPACE + w_fst), 1)
        self.w4 = pynutil.add_weight(pynini.closure(NEMO_WHITE_SPACE + w_fst), 0.5)
    ...
    def passthrough_fst(self, fst):
        return (
            pynutil.add_weight(self.middle_phrase_fst + pynutil.add_weight(fst, -10) + self.middle_phrase_fst, weight=-100)
            | pynutil.add_weight(pynini.closure(self.phrase_fst | pynutil.add_weight(fst, -10)), weight=-20)
        )
