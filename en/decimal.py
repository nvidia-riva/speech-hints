...
class Decimal:
    def __init__(self):
        ...
        decimals_graph = point + pynini.closure(
            pynutil.add_weight(delete_space + str_to_digit, -0.0001)
        )
        self.fst = pynutil.add_weight(integer_graph, -0.0001) | (
            (
                pynutil.add_weight(integer_graph, -0.0002)
                | pynutil.add_weight(str_to_digit, 1.0)
            )
