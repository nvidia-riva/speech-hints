...
class NumericSequence:
    def __init__(self, o_is_zero: bool = True):
        ...
        triple_digit_to_digit = pynini.compose(
            triple_digit,
            str_to_digit + delete_space + str_to_digit + delete_space + str_to_digit,
        )
        self.double_triple_graph = triple_digit_to_digit | double_digit_to_digit

        self.numeric_graph = pynutil.add_weight(
            (self.double_triple_graph | str_to_digit).optimize(), -0.05
        )
        ...
        sequence = (
            (
                self.numeric_graph
                + pynini.closure(
                    pynutil.add_weight(delete_spaces + self.numeric_graph, -0.001), 2
                )
            )
            | double_digit_to_digit
            | triple_digit_to_digit
        ).optimize()

        self.fst = pynini.compose(
            NEMO_WHITE_SPACE + sequence + NEMO_WHITE_SPACE,
            NEMO_WHITE_SPACE + NEMO_DIGIT ** (2, ...) + NEMO_WHITE_SPACE,
        ).optimize()
